  ...
  <div class="metrics-dashboard p-6 space-y-6 overflow-y-auto h-full">
    <header class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        <button v-for="r in ranges" :key="r.key" @click="timeRange = r.key"
          :class="[
    ...
    <div v-if="!agentId" class="flex items-center gap-4">
      <div class="glass-panel rounded-xl px-4 py-2 flex items-center gap-2">
        ...
        <select v-model="selectedAgentUuid" class="bg-transparent text-sm text-on-surface border-none outline-none font-mono">
...
const agentName = computed(() => {
  if (agentId.value) return allAgents.value.find(a => a.uuid === agentId.value)?.name || ''
  if (selectedAgentUuid.value) return allAgents.value.find(a => a.uuid === selectedAgentUuid.value)?.name || ''
  return ''
})
...
onMounted(async () => {
  ...
  try { allAgents.value = await apiFetch('/api/v1/agents?my=true') } catch (e) { /* non-fatal */ }
})
