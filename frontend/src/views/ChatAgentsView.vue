<template>
  <div class="agents-view h-full flex flex-col p-6 overflow-y-auto">
    ...
    <div class="flex items-center justify-between mb-6 shrink-0 flex-wrap gap-4">
      <div>
        <h1 class="font-sans text-2xl font-bold text-on-surface tracking-tight">Chat Lab</h1>
        <p class="font-mono text-[11px] text-on-surface-variant/60 uppercase tracking-wider mt-1">
          {{ activeNodesCount }} Active Chat Nodes • {{ tabCount('chat') }} Chat Agents Deployed
        </p>
      </div>
    ...
    <div class="flex-1 min-h-[300px]">
      ...
      <div v-else :class="[viewMode === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4' : 'flex flex-col gap-3']">
        ...
        >
          ...
          <span class="text-xs font-bold text-on-surface-variant group-hover:text-primary transition-colors">Deploy New Chat Node</span>
          <span class="text-[9px] font-mono text-on-surface-variant/60 mt-1 uppercase tracking-wider">Initialize chatbot engine</span>
        </div>
      </div>
    </div>
...
const tabs = [
  ...
  { key: 'templates', label: 'Chat Templates' },
]
...
const filteredAgents = computed(() => {
  if (activeTab.value === 'chat') return agents.value.filter(a => !a.is_voice_agent && !a.is_template)
  return agents.value.filter(a => !a.is_voice_agent && a.is_template)
})

function tabCount(key) {
  if (key === 'chat') return agents.value.filter(a => !a.is_voice_agent && !a.is_template).length
...
async function onSaved(updatedAgent) {
  ...
  await fetchAgents()
  toast.show('Chat agent configured successfully', 'success')
}
...
async function cloneAgent(agent) {
  try {
    ...
    toast.show('Chat agent duplicated successfully', 'success')
  } catch (e) {
    toast.show(e.message, 'error')
  }
}
...
async function deleteAgent(agent) {
  if (!confirm(`Are you sure you want to delete chat agent "${agent.name}"?`)) return
  try {
...
watch(() => route.query.new, (val) => {
  if (val === 'true') {
    openBuilder(null)
    router.replace({ path: '/chat-lab', query: {} })
  }
}, { immediate: true })
